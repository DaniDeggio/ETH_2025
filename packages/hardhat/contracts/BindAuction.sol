...
contract BlindAuction is SepoliaConfig, ReentrancyGuard {
    ...
    /// @param cleartexts ABI-encoded decrypted payload returned by the oracle.
    /// @param decryptionProof Signatures proving the oracle computation.
    function resolveAuctionCallback(uint256 requestId, bytes memory cleartexts, bytes memory decryptionProof) public {
        ...
        address resolvedWinnerAddress = abi.decode(cleartexts, (address));
        winnerAddress = resolvedWinnerAddress;
    }
}